...
if install_graph_nets_library.lower() == "yes":
  ...
  get_ipython().system('pip install graph_nets')
else:
...
def calculate_weights(target):
  target_nodes_sum = tf.reduce_sum(target)

  sz = tf.size(target)

  def x(v):
      if x == 1:
          return sz / target_nodes_sum
      else:
          return tf.constant(.01, dtype=tf.double)

  return tf.map_fn(x, target)


def create_loss_ops(target_op, output_ops):
  x = calculate_weights(target_op.nodes)
  y = tf.Print(target_op.nodes, [target_op.nodes], "targets", summarize= 100)
  node_weights = tf.Print(x, [x], "weights", summarize = 100)


  loss_ops = [
      tf.losses.softmax_cross_entropy(y, output_op.nodes, weights=node_weights) +
      tf.losses.softmax_cross_entropy(target_op.edges, output_op.edges, weights=calculate_weights(target_op.edges))
...
num_nodes_min_max_tr = (40, 60)
num_nodes_min_max_ge = (40, 60)
